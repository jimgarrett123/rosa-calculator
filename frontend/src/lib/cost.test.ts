...
import { getEstimate, getWorkerNodes, WorkerNode } from './cost';
...
const getMockNodes = (string: string, count: number) => {
...
describe('get worker nodes', () => {
  test('throws if price not found', () => {
    expect(
      ...
      Object {
        "error": "EC2 type \\"r5.4xlarge\\" not found in the Price API data",
      }
    `);
  });
...
describe('estimate', () => {
  test('should fail due to infra node price not found', () => {
    ...
    const workerNodes = getWorkerNodes(getMockNodes('m5.xlarge', 3), ec2Prices) as WorkerNode[];

    expect(() => getEstimate(workerNodes, 3, ec2Prices, ebsPrices)).toThrowErrorMatchingInlineSnapshot(
      `"Infra node price not found (EC2 type: r5.xlarge)"`
    );
  });
  ...
  test('should fail due to control plane node price not found', () => {
    ...
    expect(() => getEstimate(workerNodes, 3, ec2Prices, ebsPrices)).toThrowErrorMatchingInlineSnapshot(
      `"Control plane node price not found (EC2 type: m5.2xlarge)"`
    );
  });
  ...
  test('should be identical to on-demand example in ROSA pricing page https://aws.amazon.com/rosa/pricing/', () => {
    ...
    expect(getEstimate(workerNodes, 3, ec2Prices, mockEbsPrices)).toMatchInlineSnapshot(`
      Object {
        "estimate": Object {
          "controlPlane": Object {
            "1year": Object {
  ...
  test('should change the size of the control plane and infra if >25 and <= 100', () => {
    ...
    const workerNodes = getWorkerNodes(getMockNodes('m5.xlarge', 26), ec2Prices) as WorkerNode[];

    expect(getEstimate(workerNodes, 3, ec2Prices, mockEbsPrices)).toMatchObject({
      estimate: {
        controlPlane: {
  ...
  test('should change the size of the control plane and infra if >100', () => {
    ...
    const workerNodes = getWorkerNodes(getMockNodes('m5.xlarge', 101), ec2Prices) as WorkerNode[];
...
describe('input data (worker nodes', () => {
  ...
  test('fails if csv data does not contain ec2_type header in first line', () => {
    expect(
      ...
      Object {
        "error": "CSV header missing: ec2_type",
      }
    `);
  });
});
